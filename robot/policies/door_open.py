...
from spot.spot_helpers import get_current_yaw, convert_to_cv_image, grasp_object, return_to_saved_yaw, get_current_pose, \
    push_object
...
def cleanup_and_exit(spot, dock_id=521):
    ...
    try:
        ...
        try:
            ...
            spot.spot_docking.dock(dock_id=dock_id)
...
def analyze_door_joint(spot: SpotWrapper, config):
...
def execute_policy_opening(spot: SpotWrapper, config, interactive_perception):
    ...
    snapshot = spot.robot_state.kinematic_state.transforms_snapshot
    initial_hand_pose = get_a_tform_b(snapshot, VISION_FRAME_NAME, HAND_FRAME_NAME)
    initial_hand_pos = np.array([initial_hand_pose.x, initial_hand_pose.y, initial_hand_pose.z])

    current_yaw = get_current_pose(spot)[2]
    ...
    for step in range(config.max_steps):
        ...
        try:
            ...
            if interactive_perception.joint_type == "prismatic":
                ...
                push_object(spot, dx=deltas[0],dy=deltas[1],d_yaw=d_yaw,dt=3)
...
def door_open_pixel_location(spot: SpotWrapper, config, x_pix, y_pix):
    ...
    # execute policy
    try:
        saved_yaw = get_current_yaw(spot)
        ...
        return_to_saved_yaw(spot, saved_yaw)
...
def door_open(spot: SpotWrapper, config):
...
def main():
    ...
    try:
        spot = SpotWrapper(options.username, options.password, options.hostname)
        door_open(spot, options)
        return True
